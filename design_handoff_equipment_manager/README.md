...
- **Components**:
  ...
  - **Section "Thiết bị đang trang bị theo phòng ban"** (grid **2 cột**, không còn phụ đề): 4 card phòng ban — **Phòng Kinh doanh (28), Phòng Kỹ thuật (24), Phòng Kế toán (21), Phòng IT (19)**. Góc phải header section có nút **"Cấp phát"** (primary, icon +) → mở **Allocation Drawer** (§13).
    - **Mỗi card cao cố định 430px**; danh sách thiết bị bên trong tự cuộn để các card luôn thẳng hàng.
    - Header card: icon toà nhà 40px (nền `--primary-soft`, màu primary) + tên phòng (14px/700) + "% tổng cấp phát" + số lớn (28px/700) bên phải.
    - **Nhãn "PHIẾU ĐỀ NGHỊ (nn)"** kèm số lượng phiếu của phòng (pad 2 chữ số, vd `(02)`).
    - **Hàng chip phiếu**: các mã phiếu (vd `DX-300`, `DX-287`) dạng chip bo tròn (pill), font monospace 12px/700. Chip active: nền primary, chữ trắng. **Hàng chip cuộn ngang** khi tràn.
    - Dòng meta: **chỉ "{ngày}"** (đã bỏ tên người đề nghị) + **badge trạng thái chỉ hiển thị khi = "Đang trang bị"** (ẩn với "Hoàn tất").
    - **Danh sách thiết bị** của phiếu đang chọn: mỗi dòng = tên thiết bị + ngày giờ mượn (monospace 11px) + "Mượn: {người}" + "Cấp: {người cho mượn}" + **nút "Trả về" ở cuối MỌI dòng** → mở Return Dialog.
    - **Empty state**: khi phiếu không có thiết bị → icon thùng + text "Không có thiết bị được cấp phát".
    - **Phân trang**: tối đa **6 thiết bị / trang**, nếu vượt thì hiện pager "Trang x/y" + nút ‹ [số] ›. Đổi chip thì reset về trang 1.
  - _Lưu ý_: Bản hiện tại đã **bỏ** biểu đồ cột tháng và panel Cảnh báo theo yêu cầu review. Nếu cần khôi phục, xem mục Interactions.
...
- **Components**: Ô tìm kiếm (placeholder "Tìm phiếu, phòng ban…") + nút **"Tạo phiếu đề nghị"** (primary) → mở **Create Request Dialog** (§14). Bảng: Mã phiếu (monospace, primary, vd `DX-301`) | Phòng ban | Ngày lập | SL | Trạng thái (badge) | ›. Click hàng → Request Detail.
- **Trạng thái phiếu chỉ còn 2 giá trị**: "Đang trang bị" (blue) / "Hoàn tất" (green). _(Đã bỏ "Đang xử lý" và bỏ cột "Người đề nghị" toàn hệ thống.)_
...
- **Components**:
  ...
  - Grid meta **3 cột**: Phòng ban / Ngày lập / Số thiết bị. + Ghi chú. _(Đã bỏ "Người đề nghị".)_
  - **Bảng thiết bị trên phiếu**: SKU | Tên | Loại | Người nhận | Thao tác (nút "Trả về" mỗi dòng → Return Dialog).

### 7. Allocation Form (Cấp phát lẻ)

- **Topbar**: "Cấp phát lẻ" / "Bàn giao thiết bị cho nhân viên".
- **Components**: Card form max-width 720px. Các field: **Thiết bị** — _combobox đa chọn có tìm kiếm_ (token/chip): gõ tên/SKU để lọc, kết quả hiện trong dropdown nổi, chọn → thành chip xoá được ngay trong ô; chỉ liệt kê thiết bị "Trong kho", giới hạn 50 kết quả/lần. **Nhân viên nhận** (select), **Phòng ban** (select), **Ngày hẹn trả** (date), **Liên kết phiếu đề nghị** (select, optional), **Tình trạng/ghi chú** (textarea). Footer: nút Hủy + "Xác nhận cấp phát" (primary). Field bắt buộc đánh dấu `*` đỏ.
...
### 13. Allocation Drawer (Cấp phát nhanh từ Dashboard) — MỚI

Mở bằng nút **"Cấp phát"** trên section dept của Dashboard (thay cho link "Xem tất cả" cũ).

- **Side panel phải**, rộng **400px**, cao 100vh, trượt vào từ phải (animation `slideInRight`). **Không chặn nền**: overlay `pointer-events:none`, chỉ panel nhận tương tác; đồng thời `<main>` được đẩy `padding-right:444px` để mọi card phòng ban vẫn thấy & thả được khi panel mở.
- Cấu trúc panel (trên → dưới): header ("Cấp phát thiết bị" + phụ đề "Chọn rồi kéo thiết bị vào thẻ phòng ban" + nút X) → **Select "Phiếu đề nghị"** (chỉ phiếu trạng thái "Đang trang bị") → ô tìm kiếm → **danh sách thiết bị "Trong kho"** (cuộn) → footer hint kéo-thả.
- **Đa chọn**: mỗi dòng có checkbox + có "select-all" + bộ đếm "{n} đã chọn". Dòng được chọn đổi nền/viền primary.
- **Kéo & thả thẳng vào card phòng ban**: dòng thiết bị `draggable`. Kéo 1 thiết bị đã-chọn → mang **toàn bộ tập đã chọn**; kéo thiết bị chưa chọn → chỉ thiết bị đó. Card phòng ban là **drop target**: hover sáng viền + ring; thả → mở **Confirm Dialog**.
- **Confirm Dialog (sau khi thả)**: hiển thị **phòng ban đích** + **danh sách thiết bị đã thả** (cuộn, tối đa ~168px) + input **Tên người mượn** (`*`) + textarea ghi chú. Footer: Hủy / "Xác nhận cấp phát". Xác nhận → xoá selection & đóng.

### 14. Create Request Dialog (Tạo phiếu đề nghị) — MỚI

Mở bằng nút **"Tạo phiếu đề nghị"** ở màn Phiếu đề nghị. **Modal** rộng 520px (animation `popIn`).

- Header: "Tạo phiếu đề nghị" + phụ đề + X.
- Field (grid 2 cột): **Mã phiếu** (`*`, input **monospace**, placeholder "Nhập mã phiếu, VD: DX-302", full-width), **Phòng ban** (`*`, select, full-width), **Ngày lập** (date, full-width), **Ghi chú** (textarea, full-width). _(Không có field "Người đề nghị".)_
- Footer: Hủy / "Tạo phiếu" (primary, icon +).
...
- **Dashboard dept cards**: click chip phiếu → đổi danh sách thiết bị bên dưới (state per-card `deptTab`); phân trang 6/trang (state per-card `deptPage`), reset trang khi đổi chip. Nút "Trả về" có ở **mọi dòng**. Card là **drop target** của Allocation Drawer.
- **Allocation Drawer**: nút "Cấp phát" mở panel phải (`lendOpen`); tìm kiếm (`lendQuery`), đa chọn (`lendSelected`), select phiếu (`lendReq`). Kéo dòng → set `draggedDevices`; hover card set `dropDept` (ring); thả card mở `lendModal` (`{devices, dept}`) → Confirm Dialog (nhập người mượn) → "Xác nhận" reset selection.
- **Allocation Form (Cấp phát lẻ)**: combobox đa chọn thiết bị — `allocQuery` lọc, `allocOpen` mở/đóng dropdown (focus/blur, `mousedown` giữ chọn trước blur), `allocSelected` danh sách chip; thêm thiết bị → xoá query & giữ dropdown mở; bỏ chip qua nút ✕; cap 50 kết quả.
- **Create Request Dialog**: `createReqOpen` mở/đóng modal; field gồm Mã phiếu, Phòng ban, Ngày lập, Ghi chú.
...
- `lendOpen`, `lendQuery`, `lendSelected[]`, `lendReq`, `dragOver`, `dropDept`, `lendModal` — Allocation Drawer (panel phải, đa chọn, kéo-thả vào card, confirm dialog). `draggedDevices` giữ tập đang kéo (instance field, ngoài state).
- `allocQuery`, `allocSelected[]`, `allocOpen` — combobox thiết bị ở màn Cấp phát lẻ.
- `createReqOpen` — Create Request Dialog.
...
- Yellow (Đang bảo trì — trạng thái thiết bị): `#ca8a04` _(phiếu đề nghị không còn dùng "Đang xử lý")_
